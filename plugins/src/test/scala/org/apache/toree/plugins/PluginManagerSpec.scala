...
import org.clapper.classutil.{MethodInfo, FieldInfo, ClassInfo}
  ...
  describe("PluginManager") {
    ...
    describe("#initializePlugins") {
      ...
      it("should return a collection of successes for each plugin method") {
        ...
        results.head._1 should be (testPlugin.name)
        results.head._2.head shouldBe a [Success[_]]
      }
      ...
      it("should return failures for any failed plugin method") {
        ...
        results should have size 1
        results.head._1 should be (testPlugin.name)
        results.head._2.head shouldBe a [Failure[_]]
      }
    }
    ...
    describe("#fireEvent") {
      ...
      it("should return a collection of success/failure for invoked plugin methods") {
        val c = classOf[TestPlugin]
        val plugin = pluginManager.loadPlugin(c.getName, c).get
        val testPlugin = plugin.asInstanceOf[TestPlugin]
        ...
        r1.head shouldBe a [Success[_]]

        r2 should have size 1
        r2.head shouldBe a [Failure[_]]
      }
    }
  }
}
